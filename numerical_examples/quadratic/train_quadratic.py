...
from benchmarks import quadratic_function_torch
import os
...
def train_one_epoch(model, dataloader, optimizer, criterion, device):
    ...
    for batch in dataloader:
        ...
        quadratic_func = quadratic_function_torch(A=A, b=b, dim=xopt.shape[1])
        ...
        loss = criterion(quadratic_func(reconstructed_x), quadratic_func(xopt))
...
def main():
    ...
    batch_size = 64
    epochs = 100000
    checkpoint_path = f"out/quadratic/model_{input_dim}d_{latent_dim}l.pt"
    model_dir = '../../out'
    os.makedirs(model_dir, exist_ok=True)

    # Load the dataset (training and validation)
    train_data_path = f"data/quadratic/train/quadratic_{input_dim}d_0.5pct_optima_data.pt"
    valid_data_path = f"data/quadratic/valid/quadratic_{input_dim}d_0.5pct_optima_data.pt"
