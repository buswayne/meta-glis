...
def generate_optima(file_path, data_size=10000, dim=20, bounds=(-30, 30), perturb_pct=0):
    lb, ub = bounds
...
if __name__ == "__main__":
    ...
    input_dim = 20
    data_size = 50
    bounds = (-30. * np.ones(input_dim), 30. * np.ones(input_dim))
    pertub_pct = 0.1

    generate_optima("data/rosenbrock/test/rosenbrock_optima_data.pt",
                    data_size=data_size,
                    dim=input_dim,
                    bounds=bounds,
