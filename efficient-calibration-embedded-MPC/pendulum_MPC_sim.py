...
def compute_system_matrices(params):

    if isinstance(params, dict):  # Check if params is a dictionary
        params = np.array([params["M"], params["m"], params["b"],
...
def simulate_pendulum_MPC(sim_options, params):
    ...
    Ac, Bc = compute_system_matrices(params)
    ...
    system_dyn = ode(lambda t, x, u: f_ODE_wrapped(t, x, u, params=params)).set_integrator('vode', method='bdf')
...
if __name__ == '__main__':
    ...
    simout = simulate_pendulum_MPC(simopt, custom_params)
