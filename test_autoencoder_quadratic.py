...
def test_model(test_loader, model_path, input_dim, latent_dim, device):
    ...
    plt.scatter(true_optima[:, 0], true_optima[:, 1], label='true', s=5)
    plt.scatter(pred_optima[:, 0], pred_optima[:, 1], label='pred', s=5)
...
def main():
    ...
    latent_dim = 5
    batch_size = 128
    test_data_path = f"data/quadratic/test/quadratic_{input_dim}d_0.5pct_optima_data.pt"
    model_path = f"out/quadratic/model_{input_dim}d_{latent_dim}l.pt"
