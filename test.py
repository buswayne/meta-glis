...
def test_model(test_loader, model_path, input_dim, latent_dim, device):
    ...
    plt.hlines(np.ones(max_evals) * true_optima, 0, max_evals - 1, colors='blue', label="True Optimum (Mean)", alpha=0.3)
...
def main():
    ...
    latent_dim = 5
    batch_size = 128
    test_data_path = "data/rosenbrock/test/rosenbrock_optima_data.pt"
    model_path = "out/model_checkpoint.pt"
